...
MainWindow::MainWindow( QWidget *parent ) : QMainWindow(parent), m_ui(new Ui::MainWindow)
{
  ...
  connect(m_ui->m_p1,SIGNAL(valueChanged(double)),this,SLOT(setPace()));
  connect(m_ui->m_p2,SIGNAL(valueChanged(double)),this,SLOT(setPace()));
  connect(m_ui->m_p3,SIGNAL(valueChanged(double)),this,SLOT(setPace()));
  /// connect the reset button
  //connect(m_ui->m_reset,SIGNAL(clicked()),m_gl,SLOT(reset()));
  connect(m_ui->m_show_bone,SIGNAL(toggled(bool)),m_gl,SLOT(showSkeletons(bool)));
  connect(m_ui->m_resetani,SIGNAL(toggled(bool)),m_gl,SLOT(resetanimation(bool)));
  connect(m_ui->m_elimination,SIGNAL(toggled(bool)),m_gl,SLOT(elimination(bool)));
...
//----------------------------------------------------------------------------------------------------------------------
void MainWindow::setPace()
{
  m_gl->setPace(m_ui->m_p1->value(), m_ui->m_p2->value(),m_ui->m_p3->value());
}
//----------------------------------------------------------------------------------------------------------------------
